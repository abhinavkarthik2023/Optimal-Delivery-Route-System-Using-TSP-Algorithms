...
import {
  ...
  FormControl, OutlinedInput, Chip, Table, TableBody, TableCell, TableContainer,
...
export default function App() {
  ...
  const [distanceMatrix, setDistanceMatrix] = useState([]); // State to hold the distance matrix
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isStatsModalOpen, setIsStatsModalOpen] = useState(false);

  const renderMatrixTable = (matrix, nodes) => {
    return (
      <TableContainer component={Paper} sx={{ maxHeight: 440, mt: 2 }}>
        <Table stickyHeader aria-label="distance matrix">
          <TableHead>
            <TableRow>
              <TableCell align="center">/</TableCell>
              {nodes.map((node, index) => (
                <TableCell key={index} align="center">{node.id}</TableCell>
              ))}
            </TableRow>
          </TableHead>
          <TableBody>
            {matrix.map((row, idx) => (
              <TableRow key={idx}>
                <TableCell key={`head-${idx}`} align="center">{nodes[idx].id}</TableCell>
                {row.map((cell, index) => (
                  <TableCell key={`cell-${idx}-${index}`} align="center">{cell.toFixed(2)}</TableCell>
                ))}
              </TableRow>
            ))}
          </TableBody>
  ...
  const findOptimalRoute = async () => {
    ...
    if (response.length && response[0].optimized_route) {
      ...
      setDistanceMatrix(bestResult.distance_matrix); // Setting the distance matrix from the best result
    } else {
      ...
      <Box sx={{ flexGrow: 1 }}>
            ...
            >
              ...
              >
                <List>
                  ...
                  <ListItem disablePadding>
                    <ListItemButton onClick={() => { setPath([]); setOptimalPath([]); setTotalDistance(0); setAlgorithm([]); setSelectedAlgorithm(''); setExecutionData([]); setDistanceData([]); setDistanceMatrix([]); setIsDrawerOpen(false); }}>
                      <ListItemIcon>
                        <RefreshIcon />
                      </ListItemIcon>
        ...
        >
          <Box sx={style}>
            ...
            {distanceMatrix.length > 0 && renderMatrixTable(distanceMatrix, optimalPath)}
            <Typography sx={{ mt: 2 }}>
